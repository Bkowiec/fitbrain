...
const FIELD_KEY = 'dev:ddfa';

function dtAt(samples: Sample[], i: number): number {
  if (i === 0) return 0;
  return Math.max(0, Math.min(30, samples[i].timer - samples[i - 1].timer));
}
...
export function computeZoneSense(samples: Sample[], splits: Split[], splitDistance: number, session: Msg, devNames: Record<string, string>): ZoneSense | undefined {
  ...
  for (let i = 0; i < samples.length; i++) {
    const v = samples[i].extra?.[FIELD_KEY];
  ...
  // Time in zones, time-weighted like the other zone computations.
  let aerobic = 0, anaerobic = 0, vo2max = 0;
  for (const { i, v } of pts) {
    const dt = dtAt(samples, i);
    if (v < ZONESENSE_ANAEROBIC) vo2max += dt;
    else if (v < ZONESENSE_AEROBIC) anaerobic += dt;
    else aerobic += dt;
  }
  ...
  // Per split (distance based).
  const perSplit: ZoneSenseSplit[] = [];
  for (const sp of splits) {
    const inSplit = pts.filter((p) => { const d = samples[p.i].dist; return isNum(d) && d >= sp.startDist && d < sp.endDist; });
    if (inSplit.length < 5) continue;
    const vs = inSplit.map((p) => p.v);
    perSplit.push({
      index: sp.index, endDist: sp.endDist, mean: mean(vs)!,
      anaerobicPct: (vs.filter((v) => v < ZONESENSE_AEROBIC && v >= ZONESENSE_ANAEROBIC).length / vs.length) * 100,
      vo2maxPct: (vs.filter((v) => v < ZONESENSE_ANAEROBIC).length / vs.length) * 100,
  ...
  return {
    ...
    computedTimes: { aerobic, anaerobic, vo2max },
